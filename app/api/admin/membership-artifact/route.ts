...
export async function GET(request: Request) {
  try {
    ...
    const source = fs.readFileSync(path.join(process.cwd(), "contracts", "evm", "DappsterMembership.sol"), "utf8")
    return NextResponse.json(compileSolidity(source, "DappsterMembership"))
  } catch (error) {
