...
export async function GET(request: Request) {
  try {
    ...
    const source = fs.readFileSync(path.join(process.cwd(), "contracts", "DappIdentityRegistry.sol"), "utf8")
    return NextResponse.json(compileSolidity(source, "DappIdentityRegistry", { chainId: 8453 }))
  } catch (error) {
