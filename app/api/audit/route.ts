...
import { assertCredits, CREDIT_COSTS, hasActivePro } from "@/lib/credits"
...
export async function POST(request: Request) {
  try {
    ...
    const profile = user.isDemo ? { credits: localCredits(user.id), plan: "free" } : await assertCredits(user.id, cost)
    const activePro = hasActivePro(profile)
    if (!activePro && profile.credits < cost) throw new Error(`You need ${cost} credits for this action`)
    ...
    const auditId = user.isDemo
      ...
      : (await supabaseRequest<{ id: string }[]>({ path: "audits", method: "POST", body: { dapp_id: input.dappId || null, owner_id: user.id, contract_code: input.contractCode, chain: input.chain, report, severity_counts: report.severity_counts, status: "completed", credits_used: cost } }))[0]?.id
    if (input.dappId) {
    ...
    return NextResponse.json({ auditId, report, creditsRemaining, mode: user.isDemo ? "local" : "supabase", disclaimer: "This AI audit is not a replacement for a professional human security audit. Always review code manually before deploying to mainnet." })
  } catch (error) {
