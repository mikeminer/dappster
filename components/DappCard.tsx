...
export function DappCard({ dapp }: { dapp: Dapp }) {
  return (
    <article className="dapp-card">
      ...
      <div className="dapp-meta"><div className="tags">{dapp.tags.slice(0, 2).map(tag => <span className="tag" key={tag}>{tag}</span>)}</div><span className="owner">{dapp.owner}</span></div>
