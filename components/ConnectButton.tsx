...
function ConnectButtonCore({ mode = "button", redirectTo, reown }: { mode?: "button" | "panel"; redirectTo?: string; reown: ReownConnectionState | null }) {
  ...
  const [dappsterPoints, setDappsterPoints] = useState(0)
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      if (response.ok) {
        const payload = await response.json() as { wallets?: LinkedWallet[]; dappsterPoints?: number }
        if (active) {
          setLinkedWallets(payload.wallets || [])
          setDappsterPoints(payload.dappsterPoints || 0)
        }
      }
    }
    void sync()
  ...
  return <><button className="btn btn-outline" onClick={() => { setMessage(""); setOpen(true) }}><span aria-hidden="true">◇</span> Connect wallet</button>{open && createPortal(<div className="modal-backdrop wallet-modal-backdrop" onMouseDown={() => setOpen(false)}><div className="modal" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="Connect wallet"><div className="panel-head"><div><div className="panel-title">Connect your wallet</div><div style={{color:"#737b85",fontSize:12,marginTop:5}}>Choose a network to sign in to Dappster.</div></div><button className="btn btn-ghost" onClick={() => setOpen(false)}>×</button></div><div className="panel-body">{options}</div></div></div>, document.body)}</>
}
