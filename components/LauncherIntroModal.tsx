...
export function LauncherIntroModal() {
  ...
  useEffect(() => {
    try {
      const previewRequested = new URLSearchParams(window.location.search).get("intro") === "1"
      if (previewRequested || sessionStorage.getItem(SESSION_KEY) !== "dismissed") setOpen(true)
    } catch {
      setOpen(true)
    }
  }, [])
  ...
  return (
    <div className="modal-backdrop launcher-intro-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) close() }}>
      <section className="modal launcher-intro-modal" role="dialog" aria-modal="true" aria-labelledby="launcher-intro-title" aria-describedby="launcher-intro-description">
        ...
        <header className="launcher-intro-header">
          <div className="launcher-intro-mark" aria-hidden="true"><span>D</span></div>
          <div className="launcher-intro-heading">
        ...
        <div className="launcher-intro-creator-row">
          ...
          <a className="launcher-intro-token" href={CREATOR_TOKEN_URL} target="_blank" rel="noopener noreferrer">
            Discover the creator token<ArrowRight size={14} aria-hidden="true" />
          </a>
        </div>
        <div className="launcher-intro-benefits" aria-label="Token holder deployment benefits">
          <div className="launcher-intro-section-heading">
            <span className="launcher-intro-section-icon"><Sparkles size={16} aria-hidden="true" /></span>
            <span><small>DEVELOPER TOKEN HOLDER BENEFITS</small><strong>Hold tokens. Build more.</strong></span>
          </div>
          <p className="launcher-intro-benefits-title">Qualifying linked wallets unlock unlimited generation and security audits for their ecosystem.</p>
          <div className="launcher-intro-benefits-grid">
            {DEPLOYMENT_BENEFITS.map(benefit => (
              <a className="launcher-intro-benefit" href={benefit.href} target="_blank" rel="noopener noreferrer" key={benefit.ecosystem}>
                <span className="launcher-intro-benefit-topline">
                  <span className="launcher-intro-benefit-logo"><Image src={benefit.logo} alt="" width={24} height={24} /></span>
                  <span>{benefit.ecosystem}</span>
                  <small>HOLDER BENEFIT</small>
                </span>
                ...
                <span className="launcher-intro-benefit-action">{benefit.action}<ExternalLink size={13} aria-hidden="true" /></span>
              </a>
            ))}
          </div>
        </div>
        <a className="launcher-intro-contract" href={CONTRACT_URL} target="_blank" rel="noopener noreferrer">
          ...
          <span><small><ShieldCheck size={12} aria-hidden="true" /> VERIFIED SUBSCRIPTION CONTRACT</small><strong>Subscriptions are based on Base</strong><code>{SUBSCRIPTION_CONTRACT.slice(0, 10)}…{SUBSCRIPTION_CONTRACT.slice(-8)}</code></span>
