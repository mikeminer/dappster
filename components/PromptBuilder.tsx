...
export function PromptBuilder() {
  ...
  function startOrResumeDeployment() {
    if (deployStage === "funding-ready") {
      const resumeFunding = solanaFundingApprovalRef.current
    ...
    <div className="builder-grid">
      <section className="panel">
        ...
        <div className="panel-body form-stack">
          ...
          {chain === "solana" && <div><label className="form-label" htmlFor="solana-network">Solana deployment cluster</label><select id="solana-network" className="select" value={solanaCluster} disabled={Boolean(deployStage || contractDeployment)} onChange={event => setSolanaCluster(event.target.value as SolanaDeploymentCluster)}><option value="devnet">Devnet · recommended for testing</option><option value="mainnet-beta">Mainnet</option></select><div style={{color:"#707883",fontSize:10,lineHeight:1.5,marginTop:7}}>Selected cluster: <strong>{solanaClusterName}</strong>. Dappster verifies its genesis hash before Phantom signs anything and binds the funding request to <span className="mono">{solanaWalletChain}</span>. {solanaCluster === "devnet" ? <>Enable Phantom <strong>Settings → Developer Settings → Testnet Mode</strong>, then reconnect. Mainnet SOL is never used for a Devnet deployment. <a href="https://docs.phantom.com/developer-powertools/testnet-mode" target="_blank" rel="noreferrer">Phantom guide</a>.</> : <>Phantom must expose a Mainnet account and the wallet must contain real Mainnet SOL. Devnet SOL is never accepted for a Mainnet deployment.</>}</div></div>}
          ...
          {generation && chain === "solana" && !contractDeployment && <div className="recovery-box"><div><strong>User-funded Solana deployment · {solanaClusterName}</strong><small style={{display:"block",marginTop:6,lineHeight:1.5}}>Press <span className="mono">Deploy program + frontend</span>. The quote, funding transaction, deployment worker and Program ID verification are all restricted to <strong>{solanaClusterName}</strong>. {solanaCluster === "devnet" ? "Phantom Testnet Mode is required and only Devnet SOL is accepted." : "Phantom must use Solana Mainnet and only real Mainnet SOL is accepted."}</small></div></div>}
      ...
      <section className="panel code-window">
        <div className="panel-head">
          ...
          {generation && <div className="builder-deploy-actions"><button className="btn btn-ghost" onClick={() => setPreviewOpen(true)}><Eye size={14} /> Preview</button><button className="btn btn-ghost" onClick={copy}>{copied ? <Check size={14} /> : <Copy size={14} />}{copied ? "Copied" : "Copy"}</button>{deployment ? <a className="btn btn-primary builder-deploy-button" href={deployment.url} target="_blank" rel="noreferrer">Open dApp <ExternalLink size={14} /></a> : <button className="btn btn-primary builder-deploy-button" aria-busy={Boolean(deployStage && deployStage !== "funding-ready" && deployStage !== "authorization-ready")} disabled={(Boolean(deployStage) && deployStage !== "funding-ready" && deployStage !== "authorization-ready") || !selectedAdapter.deploymentReady} onClick={startOrResumeDeployment}>{deployStage && deployStage !== "funding-ready" && deployStage !== "authorization-ready" ? <Loader2 className="animate-spin builder-deploy-spinner" size={14} /> : <Rocket size={14} />}<span className="builder-deploy-label">{stageLabel(deployStage, chain, solanaCluster)}</span></button>}</div>}
        </div>
